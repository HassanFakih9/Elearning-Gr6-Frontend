...
// function LanguageSelection({ onSelectLanguageLevel }) {  
function LanguageSelection({ userId }) {
    ...
    ////////////////fetching the enrolled levels
    useEffect(() => {
        if (userId) {
            fetch(`http://localhost:5000/students/enrolled-levels?userId=${userId}`)
                .then((response) => response.json())
                .then((data) => {
